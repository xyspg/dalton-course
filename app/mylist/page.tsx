...
const MyList = () => {
  ...
  const [loadingLocalStorage, setLoadingLocalStorage] = useState<boolean>(false)
  ...
  const { data, error, isLoading, mutate } = useSWR(!!courses ? url : null, fetcher);
  if (isLoading || !loadingLocalStorage) return <div>Loading...</div>;
  if (!courses) {
    return (
        <div className='flex flex-col justify-center items-center mt-24'>
          <p className='text-neutral-700'>You don&apos;t have saved courses.</p>
          <p className='text-neutral-700 mb-4'>Click on &quot;Add&quot; to add a course.</p>
  ...
  return (
    <>
      ...
      {data && <DataTable columns={columns} data={courseData}  />}
    </>
  );
};
